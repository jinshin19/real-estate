...
@Component({
  ...
  template: `
    <div
      ...
      [style.height]="fullHeight() ? 'inherit' : ''"
    >
...
export class HlmCarousel {
  ...
  public readonly isCustom = input(false);
  public readonly selectedImageIndex = input<number>(0);
  ...
  protected onEmblaEvent(event: EmblaEventType) {
    ...
    if (this.isCustom() && event === 'init') {
      emblaApi.scrollTo(this.selectedImageIndex(), true);
    }
  ...
  scrollTo(index: number) {
    this._emblaCarousel().emblaApi?.scrollTo(index);
  }
}
