    ...
    @if (collapsible() === 'none') {
      ...
      >
        <hlm-sheet-content
          *hlmSheetPortal="let ctx"
          ...
          data-mobile="true"
          [class]="_mobileSidebarClassComputed()"
      ...
      >
        <div
          ...
          [class]="_desktopSidebarClassComputed()"
        >
          <ng-container *ngTemplateOutlet="contentContainer" />
        </div>
      </div>
...
export class HlmSidebar {
  ...
  // Custom
  public readonly isFixed = input<boolean>(true);
  ...
  // Custom
  public readonly desktopSidebarClass = input<ClassValue>('');
  protected readonly _desktopSidebarClassComputed = computed(() =>
    hlm(
      'bg-sidebar text-sidebar-foreground flex size-full flex-col',
      ...
      this.desktopSidebarClass(),
    ),
  );
  public readonly mobileSidebarClass = input<ClassValue>('');
  protected readonly _mobileSidebarClassComputed = computed(() =>
    hlm(
      'bg-sidebar text-sidebar-foreground h-svh w-(--sidebar-width) p-0 [&>button]:hidden',
